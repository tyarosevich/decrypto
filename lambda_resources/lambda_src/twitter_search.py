...
def tweet_handler(query, ret_max, max_request_results):
    '''
    A top level wrapper to retrieve tweets. Many parameters are hard coded for the use-case, see docs for details.
    :param query: str
    :param ret_max: int
    :param max_request_results: int
    :return: DataFrame
    '''

    twitter_api_info = get_secret('twitter_api_info')
    ...
    dct_params = {
        ...
        'end_time': None,
        'expansions': None,
        'max_results': max_request_results,
        'next_token': None,
    ...
    df_tweet_batch = _get_tweets(client, query, ret_max, dct_params)

    return df_tweet_batch

def _api_request(client, query, per_hour_max, dct_params):
    """
...
def _get_tweets(client, query, ret_max, dct_params):
    """
    ...
    for start_time, end_time in _get_start_stop()[0:4]:
        dct_params['start_time'] = start_time
        dct_params['end_time'] = end_time
        dct_params['next_token'] = None
        lst_results += _api_request(client, query, per_hour_max, dct_params)
