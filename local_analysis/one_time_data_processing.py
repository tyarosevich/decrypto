...
hourly_bitcoin_path = Path(tabular_folder + 'Bitfinex_BTCUSD_1h.csv')
hourly_ether_path = Path(tabular_folder + 'Bitfinex_ETHUSD_1h.csv')

df_ether_hourly = pd.read_csv(hourly_ether_path, header=1)
df_ether_hourly['date'] = pd.to_datetime(df_ether_hourly['date']).dt.tz_localize('UTC')
df_bitcoin_hourly = pd.read_csv(hourly_bitcoin_path, header=1)
df_bitcoin_hourly['date'] = pd.to_datetime(df_bitcoin_hourly['date']).dt.tz_localize('UTC')
...
sol_path = Path(tabular_folder + 'Bitfinex_SOLUSD_1h.csv')
# usdc_path = Path(tabular_folder + 'Gemini_USDCUSD_1h.csv')
ust_path = Path(tabular_folder + 'Bitfinex_USTUSD_1h.csv')
# df_link = pd.read_csv(link_path, low_memory=False)
df_sol = pd.read_csv(sol_path, low_memory=False, header=1)
# df_usdc = pd.read_csv(usdc_path, low_memory=False)
df_ust = pd.read_csv(ust_path, low_memory=False, header=1)
for df_curr in [df_ust, df_sol]:
    df_curr['date'] = pd.to_datetime(df_curr['date']).dt.tz_localize('UTC')
...
coin_suffixes = ['BTC', 'ETH', 'SOL', 'UST']
all_crypto_frames = [df_bitcoin_hourly, df_ether_hourly, df_sol, df_ust]
...
df_bitcoin_hourly, df_ether_hourly, df_sol, df_ust = all_crypto_frames
...
df_ust_market_indices_merged = merge_index_into_crypto(df_ust, index_frames)

# Need to add the day of the week to each dataframe also. This is stupid, but a loop doesn't work
cols = list(df_btc_market_indices_merged.columns)
df_btc_market_indices_merged['day_of_week'] = df_btc_market_indices_merged['date'].dt.dayofweek
new_cols = cols[0:2] + ['day_of_week'] + cols[2:]
df_btc_market_indices_merged = df_btc_market_indices_merged[new_cols]

cols = list(df_eth_market_indices_merged.columns)
df_eth_market_indices_merged['day_of_week'] = df_eth_market_indices_merged['date'].dt.dayofweek
new_cols = cols[0:2] + ['day_of_week'] + cols[2:]
df_eth_market_indices_merged = df_eth_market_indices_merged[new_cols]

cols = list(df_sol_market_indices_merged.columns)
df_sol_market_indices_merged['day_of_week'] = df_sol_market_indices_merged['date'].dt.dayofweek
new_cols = cols[0:2] + ['day_of_week'] + cols[2:]
df_sol_market_indices_merged = df_sol_market_indices_merged[new_cols]

cols = list(df_ust_market_indices_merged.columns)
df_ust_market_indices_merged['day_of_week'] = df_ust_market_indices_merged['date'].dt.dayofweek
new_cols = cols[0:2] + ['day_of_week'] + cols[2:]
df_ust_market_indices_merged = df_ust_market_indices_merged[new_cols]




for df_curr, suffix in zip([df_btc_market_indices_merged, df_eth_market_indices_merged, df_sol_market_indices_merged, df_ust_market_indices_merged], coin_suffixes):
